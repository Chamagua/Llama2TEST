...
def descargar_imagenes_desde_pagina(url, carpeta_destino):
    ...
    if response.status_code == 200:
        ...
        imagenes = soup.find_all('img', {'src': re.compile('.+\.(png|jpg|jpeg|gif)')})
...
def encontrar_url(texto):
    # Patrón de expresión regular para buscar URLs en el texto
  ...
  def main(model_name=None, file_name=None, prompt=None):
    ...
    prompt = input("Por favor, ingrese el prompt para iniciar el chat: ")
    urls = encontrar_url(prompt)    
    print("URLs encontradas:")
    for url in urls:
        descargar_imagenes_desde_pagina(url)
        print(url)
...
def run_chat(model, tokenizer, is_chat_model, model_type, history):
    if is_chat_model:
        instruction = format_to_llama_chat_style(history)
    else:
        instruction = history[-1][0]
